...
class SAGEnet(BaseNetwork):
    ...
    def __init__(self, num_layers, dim_node, dim_edge, dim_hidden, aggr):
        ...
        for _ in range(self.num_layers):
            self.gconvs.append(SAGE_(**gconv_kwargs))

        # self.node_nn2 = SAGEConv(2 * dim_node, 2 * dim_node, normalize=False, bias=True)
        self.node_nn2 = SAGEConv(dim_node, dim_node, normalize=False, bias=True)

        # self.prop = build_mlp([2 * dim_node, dim_node + dim_node, dim_node], batch_norm=False, final_nonlinearity=False)
    ...
    def forward(self, node_feature, edge_feature, edges_indices):
        for i in range(self.num_layers):
            gconv = self.gconvs[i]
            node_feature_2, edge_feature = gconv(node_feature, edge_feature, edges_indices)
...
if __name__ == '__main__':
    ...
    model = SAGEnet(num_layers, dim_node=dim_node, dim_edge=dim_edge, dim_hidden=dim_hidden)
    # print("model", model)
