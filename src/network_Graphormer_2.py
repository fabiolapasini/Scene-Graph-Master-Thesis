...
###################################################################
    ...
    #######################################################
        ...
        # print(type(rel_pos_bias), rel_pos_bias.size())                                      # <class 'torch.Tensor'> torch.Size([28, 4, 28])
...
class MultiHeadedEdgeAttention(torch.nn.Module):
    def __init__(self, num_heads: int, dim_node: int, dim_edge: int, dim_atten: int,
        ...
        self.linear_k = nn.Linear(dim_edge, dim_edge)
        self.linear_q = nn.Linear(dim_node, dim_node)
        self.linear_v = nn.Linear(dim_node, dim_atten)

    # (node, edge, neighbour)
    def forward(self, q, k, v, attn_bias):
        batch_dim = q.size(0)
        edge_feature = self.nn_edge(torch.cat([q, k, v], dim=1))  # .view(b, -1, 1)

        # projecting
        v = self.linear_v(v).view(batch_dim, -1, self.num_heads, self.d_n)
        # v = self.linear_v(v)
        q = self.linear_q(q).view(batch_dim, -1, self.num_heads, self.d_o)
        # q = self.linear_q(q).view(batch_dim, self.d_n, self.num_heads)
        k = self.linear_k(k).view(batch_dim, -1, self.num_heads, self.d_o)
        # k = self.linear_k(k).view(batch_dim, self.d_e, self.num_heads)
        ...
        attention = torch.matmul(q* self.scaling, k)  # [b, h, q_len, k_len]
        print("attention: ", attention.size())                   # attention:  torch.cuda.FloatTensor  attention:  torch.Size([420, 64, 4])
        ...
        # x = torch.einsum('bm,bm->bm', attention.reshape_as(v), v)
...
class EncoderLayer(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_feature):
        '''print("attn_bias net 2: ", self.attn_bias.size())
        print(self.attn_bias)
        print("\n")'''
