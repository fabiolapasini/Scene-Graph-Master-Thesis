...
class SGFN():
    ...
    def train(self):
        ...
        # print("#####################################")
        # print(f"Training set len {len(self.dataset_train)}, batch size {self.batch_size}")
        # print("#####################################")
        ...
        eva_tool = util_eva.EvalSceneGraph(self.dataset_train.classNames, self.dataset_train.relationNames, multi_rel_outputs=0.5,
                                           k=0, multi_rel_prediction=self.model.mconfig.multi_rel_outputs)
        eva_tool_prob = [util_eva.EvaPairWeight(self.dataset_valid.classNames) for i in range(self.model.mconfig.N_LAYERS)]
        while(keep_training):
            print('\n\nTraining epoch: %d' % epoch)

            #  floating point number expressed in seconds (:3600 dopo per convertire in ore)
            if epoch == 1:
            ...
            for items in loader:
                ...
                if self.use_edge_descriptor:
                    ...
                    # Am I calling the __getitem__(0) in the init close to train dataset ?
                ...
                # time here
                    ...
                    # Here we are passing some parameters that are going to be passed in the process method of the SGPN algo (in this else statement)
                    # print("obj_points shape1: ", obj_points.size())         # obj_points shape1:  torch.Size([28, 3, 128])        # [num_objects, num_points, dim_point]
                    # print("rel_points shape1: ", rel_points.size())         # torch.Size([420, 4, 256])
                    # print("edges shape1: ", edge_indices.size())            # torch.Size([2, 420])
                    logs, pred_obj_cls, pred_rel_cls, probs = self.model.process(obj_points, rel_points, edge_indices.t().contiguous(), gt_obj_cls, gt_rel_cls,
                                                                                  weights_obj=self.dataset_train.w_cls_obj,
                ...
                logs = [("Misc/epo", int(epoch)),
                        ("Misc/it", int(iteration)),] + logs
    ...
    def validation(self, debug_mode = False):
        ...
        for i, items in enumerate(val_loader, 0):
            ...
            if self.use_edge_descriptor:
                ...
                # it has to process the item one at a time
                obj_points, rel_points, edge_indices, gt_obj_cls, gt_rel_cls = self.data_processing(items[2:])                
            tock = time.time()
    ...
    def eval(self, debug_mode=False):
        ...
        # Here we are finally writing on a file, before we olny wrote on log for tensorboard visualization
        print(eva_tool.gen_text())
        ###### self.model.iteration ######## name of the new folder
