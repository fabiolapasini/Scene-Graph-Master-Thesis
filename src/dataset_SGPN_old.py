...
from utils import util_ply, util_data, define, util
...
class RIODatasetGraph(data.Dataset):
    def __init__(self,
        ...
        #print(self.root)            # ../home/fabiola/Desktop/Tesi/Thesis/3DSSG
        ...
        if isinstance(self.root, list):
            ...
            with open(os.path.join(self.root, 'objects.json'), 'r') as f:
                data = json.load(f)
                #self.label_type = data['label_type']
                # self.isV2 = data['v2'] > 0
            ...
            classNames, relationNames, data1, selected_scans = \
    ...
    # scan[object] and scan[split] don't exist
    def read_relationship_json(self, data, selected_scans:list):
        ...
        for scan in data['scans']:
            if scan["scan"] == '0a4b8ef6-a83a-21f2-8672-dce34dd0d7ca':        #'fa79392f-7766-2d5c-869a-f5d6cfb62fc6':

                #print("reading scan: " , scan["scan"])
                #if self.mconfig.label_file == "labels.instances.align.annotated.v2.ply":
                if self.mconfig.label_file == "boh.ply":
                    '''
...
if __name__ == '__main__':
    ...
    config.dataset.root = '../3DSSG' #'../data/example_data'
